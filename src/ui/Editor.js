...
export class Editor {
  ...
  _buildHoly() {
    ...
    R(ground, c, 'brandRadius', 0.1, 12, 0.05, 'brand radius');
    R(ground, c, 'brandLife', 0.1, 12, 0.05, 'brand lifetime');
    R(ground, c, 'brandIntensity', 0, 3, 0.01, 'brand intensity');
    R(ground, c, 'shockRadius', 0.5, 30, 0.1, 'shockwave radius');
    ground.addColor(c, 'colorBrandA').name('brand light');
    ground.addColor(c, 'colorBrandB').name('brand warm');
    ground.addColor(c, 'colorShockA').name('shockwave ring');
    ground.addColor(c, 'colorShockB').name('shockwave crest');

    const sky = folder.addFolder('Sky judgment (lightning)');
    R(sky, c, 'skyHeight', 4, 48, 0.1, 'start height');
    R(sky, c, 'skyEndHeight', 0, 3, 0.01, 'ground height');
    R(sky, c, 'skyStrikeTime', 0.02, 1.5, 0.01, 'slam-down time');
    R(sky, c, 'skyStrands', 1, 24, 1, 'filaments');
    R(sky, c, 'skySpread', 0, 8, 0.05, 'fan at ground');
    R(sky, c, 'skySpreadNear', 0, 4, 0.05, 'fan in sky');
    R(sky, c, 'skySpreadCurve', 0.2, 4, 0.01, 'fan curve');
    R(sky, c, 'skyTwist', -4, 4, 0.01, 'twist');
    R(sky, c, 'skyTwistSpeed', -8, 8, 0.05, 'twist speed');
    R(sky, c, 'skyBranchDim', 0, 1, 0.01, 'outer dim');
    R(sky, c, 'skyJitter', 0, 4, 0.01, 'kink amplitude');
    R(sky, c, 'skyJitterScale', 0.05, 4, 0.01, 'kinks / metre');
    R(sky, c, 'skyOctaves', 1, 5, 1, 'octaves');
    R(sky, c, 'skyJitterFalloff', 0.1, 0.95, 0.01, 'octave falloff');
    R(sky, c, 'skyCrawl', -20, 20, 0.1, 'kink crawl');
    R(sky, c, 'skyPinch', 0.01, 0.4, 0.005, 'end pinch');
    R(sky, c, 'skyConverge', 0, 1, 0.01, 'lock onto ground');
    R(sky, c, 'skyWidth', 0.01, 0.8, 0.005, 'width high up');
    R(sky, c, 'skyWidthTip', 0.05, 3, 0.01, 'width at ground');
    R(sky, c, 'skyWidthCurve', 0.1, 4, 0.01, 'width curve');
    R(sky, c, 'skyCoreWidth', 1, 6, 0.05, 'spine thickness');
    R(sky, c, 'skyCoreSharp', 0.5, 12, 0.05, 'core sharpness');
    R(sky, c, 'skyGlowWidth', 1, 30, 0.1, 'halo width');
    R(sky, c, 'skyGlowFalloff', 0.2, 8, 0.05, 'halo falloff');
    R(sky, c, 'skyGlowOpacity', 0, 2, 0.01, 'halo opacity');
    R(sky, c, 'skyGlow', 0, 10, 0.05, 'glow');
    R(sky, c, 'skyRestrike', 0.5, 60, 0.5, 'restrikes / sec');
    R(sky, c, 'skyFlicker', 0, 1, 0.01, 'brightness stutter');
    R(sky, c, 'skyFlickerSpeed', 1, 90, 1, 'stutter rate');
    R(sky, c, 'skyStrandFlash', 0, 1, 0.01, 'filament blink');
    R(sky, c, 'skyTipGlow', 0, 8, 0.05, 'leading-edge glow');
    R(sky, c, 'skyTipLength', 0.005, 0.4, 0.005, 'leading-edge length');
    R(sky, c, 'skySparkRate', 0, 1200, 1, 'spark rate along bolt');
    R(sky, c, 'skyBurstSparks', 0, 500, 1, 'landing spark burst');
    R(sky, c, 'skyArcRadius', 0.2, 16, 0.1, 'ground arc radius');
    R(sky, c, 'skyArcLife', 0.1, 8, 0.05, 'ground arc life');
    R(sky, c, 'skyArcIntensity', 0, 3, 0.01, 'ground arc intensity');
    R(sky, c, 'skyArcBranches', 0, 3, 0.01, 'ground arc detail');
    sky.addColor(c, 'colorSkyCore').name('core');
    sky.addColor(c, 'colorSkyInner').name('inner');
    sky.addColor(c, 'colorSkyOuter').name('outer');
    sky.addColor(c, 'colorSkyHalo').name('halo');
    sky.addColor(c, 'colorSkyArc').name('ground arc');
    sky.addColor(c, 'colorSkyEmber').name('ground ember');
    sky.addColor(c, 'colorSkyBurstA').name('storm shell A');
    sky.addColor(c, 'colorSkyBurstB').name('storm shell B');
    sky.addColor(c, 'colorSkyBurstC').name('storm shell C');

    const pillar = folder.addFolder('Impact pillar (god-rays)');
    R(pillar, c, 'pillarHeight', 1, 24, 0.1, 'ray height');
    R(pillar, c, 'pillarRays', 1, 16, 1, 'god-rays');
    R(pillar, c, 'pillarSpread', 0, 6, 0.01, 'ray fan');
